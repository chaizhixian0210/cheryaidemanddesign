...
# --- ⭐⭐⭐ NEW AND IMPROVED ANALYSIS FUNCTION V2 ⭐⭐⭐ ---
def analyze_comments_with_deepseek(comments, persona_key):
    ...
    system_prompt = f"""
    你是一位顶级的汽车行业市场分析师和AI创意总监。你的任务是分析关于'{persona_name}'这个潜在用户群体的用户评论。
    请完成两项任务:
    1. 总结出一个详细的用户画像JSON对象, 包含'tags', 'pain_points', 'influencers'。
    2. 根据这个用户画像, 创作一个包含三段专业英文提示词(prompt)的JSON对象, 分别用于生成渲染图(rendering), 草图(sketch), 和内饰图(interior)。

    请严格按照以下JSON格式输出，不要添加任何 markdown 标记(`json`字样)或额外的解释文字:
    {{
      "persona_analysis": {{
        "tags": ["一个词或短语的标签"],
        "pain_points": ["用户抱怨的具体痛点"],
        "influencers": ["影响用户决策的渠道"]
      }},
      "image_prompts": {{
        "rendering": "一段用于生成渲染图的、专业的英文AI绘画提示词...",
        "sketch": "一段用于生成黑白草图的、专业的英文AI绘画提示词...",
        "interior": "一段用于生成内饰图的、专业的英文AI绘画提示词..."
      }}
    }}
    """
...
def generate_with_stability(prompt):
    if not STABILITY_API_KEY: st.error("Stability AI API Key 未配置。"); return None
    try:
...
def run_step_0_intro():
    ...
    if not api_ready: st.error("错误：请在Streamlit Cloud的Secrets中配置好DeepSeek和Stability AI的API Key！")
...
def run_step_3_prompt_generation():
    ...
    image_type_selection = st.radio("选择设计图类型:", list(image_type_map.keys()), horizontal=True)
    ...
    final_prompt = image_prompts.get(selected_prompt_key, "AI未能生成此类型的Prompt，请手动输入。")

    st.session_state.prompt = st.text_area("最终AI设计指令:", value=final_prompt, height=150)
